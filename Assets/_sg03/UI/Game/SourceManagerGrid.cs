...
namespace SG03.UI
{
    public class SourceManagerGrid : SaiBehaviour
    {
        ...
        private void InitializeGrid()
        {
            ...
            {
                Debug.LogError(this.transform.name + ": SourceManagerGrid requires a UIDocument reference.", this.gameObject);
                return;
            }
        ...
        {
            this.panel = root?.Q("SourceGridPanel");
            this.content = root?.Q("SourceGridContent");
            this.alphaVoidCountLabel = root?.Q<Label>("AlphaSourceCountLabel");
            this.omegaVoidCountLabel = root?.Q<Label>("OmeraSourceCountLabel");
            this.titleLabel = root?.Q<Label>("SourceGridTitle");
            this.pageLabel = root?.Q<Label>("SourceGridPageLabel");
            this.sortField = root?.Q<DropdownField>("SourceGridSortField");
            this.closeButton = root?.Q<Button>("SourceGridCloseButton");
            this.previousButton = root?.Q<Button>("SourceGridPreviousButton");
            this.nextButton = root?.Q<Button>("SourceGridNextButton");
        }
        ...
        private bool InitializeDimLayer(VisualElement root)
        {
            ...
            {
                Debug.LogError(this.transform.name + ": SourceManagerGrid requires a ModalDimLayer child.", this.gameObject);
                return false;
            }
        ...
        {
            evt.StopPropagation();
            int pageCount = this.GetPageCount(this.GetSourceCards().Count);
        ...
        private void Refresh()
        {
            ...
            List<BattleCardSlot> cards = this.GetSourceCards();
        ...
        private List<BattleCardSlot> GetSourceCards()
        {
            List<BattleCardSlot> cards = new List<BattleCardSlot>();
            BattleCardSlot[] slots = this.voidOwner == Owner.omega
                ? this.battleStateCtrl?.BattleState?.OmegaTheSource
                : this.battleStateCtrl?.BattleState?.AlphaTheSource;
        ...
        private void BuildPage(List<BattleCardSlot> cards)
        {
            ...
            if (cards.Count == 0)
            {
                Label emptyLabel = new Label($"{this.GetVoidOwnerLabel()}'s Source is empty.");
