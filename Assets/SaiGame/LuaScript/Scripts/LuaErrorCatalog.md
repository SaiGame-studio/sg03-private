...
| `ERR_ABILITY_CROSS_GUARD_NO_AZURE` | `cross_guard requires azure_blade in front_line` | Cross Guard requires Azure Blade on the front line. | Thuẫn Hộ Vệ yêu cầu Azure Blade ở hàng trước. | `lib_ability_human.lua` |
...
| `ERR_ABILITY_SILENT_STRIKE_SOURCE_NOT_ON_LINE` | `silent_strike source card is not on a battle line` | Silent Strike must be activated from a battle line. | Silent Strike phải được kích hoạt từ một hàng trên sân đấu. | `lib_ability_natureborn.lua` |
| `ERR_ABILITY_SILENT_STRIKE_NO_BAO` | `silent_strike requires untriggered bao in front_line` | Silent Strike requires an untriggered Bao on your front line. | Silent Strike yêu cầu Bao chưa kích hoạt ở hàng trước. | `lib_ability_natureborn.lua` |
| `ERR_ABILITY_SILENT_STRIKE_BAO_DEF_MISSING` | `silent_strike requires Bao item definition` | Silent Strike could not find Bao's card definition. | Silent Strike không tìm thấy định nghĩa thẻ Bao. | `lib_ability_natureborn.lua` |
| `ERR_ABILITY_FOR_BAO_SOURCE_NOT_ON_LINE` | `for_bao source card is not on a battle line` | For Bao must be activated from a battle line. | For Bao phải được kích hoạt từ một hàng trên sân đấu. | `lib_ability_natureborn.lua` |
| `ERR_ABILITY_FOR_BAO_NO_SAPPHIRE` | `for_bao requires untriggered sapphire in front_line` | For Bao requires an untriggered Sapphire on your front line. | For Bao yêu cầu Sapphire chưa kích hoạt ở hàng trước. | `lib_ability_natureborn.lua` |
...
| `ERR_ABILITY_ABYSSAL_MIST_BACKLINE_REQUIRED` | `abyssal_mist requires source card in own backline` | Abyssal Mist must be on its owner's back line. | Abyssal Mist phải ở hậu tuyến phe sở hữu. | `lib_ability_aura.lua` |
| `ERR_ABILITY_ABYSSAL_MIST_MISTHY_KILL_REQUIRED` | `abyssal_mist requires misthy to defeat an enemy` | Abyssal Mist requires Misthy to defeat an enemy. | Abyssal Mist yêu cầu Misthy kết liễu một kẻ địch. | `lib_ability_aura.lua` |
...
| `ERR_ABILITY_BLOOD_MIST_SOURCE_INVALID` | `blood_mist source card is not on the battlefield` | Blood Mist must be activated from the battlefield. | Blood Mist phải được kích hoạt trên bàn đấu. | `lib_ability_aura.lua` |
| `ERR_ABILITY_BLOOD_MIST_BACKLINE_REQUIRED` | `blood_mist requires source card in own backline` | Blood Mist must be on its owner's back line. | Blood Mist phải ở hậu tuyến phe sở hữu. | `lib_ability_aura.lua` |
| `ERR_ABILITY_BLOOD_MIST_ALREADY_ACTIVE` | `blood_mist is already active` | Blood Mist is already active. | Blood Mist đang hoạt động. | `lib_ability_aura.lua` |
| `ERR_ABILITY_BLOOD_MIST_TARGET_INVALID` | `blood_mist requires Mireya as an allied target` | Blood Mist requires selecting allied Mireya. | Blood Mist yêu cầu chọn Mireya cùng phe. | `lib_ability_aura.lua` |
| `ERR_ABILITY_BLOOD_MIST_MIREYA_TRIGGERED` | `blood_mist requires untriggered Mireya in own front_line` | Blood Mist requires an untriggered allied Mireya on the front line. | Blood Mist yêu cầu Mireya cùng phe chưa kích hoạt ở hàng trước. | `lib_ability_aura.lua` |
| `ERR_ABILITY_BLOOD_MIST_NO_BLOOD_SPIRE` | `blood_mist requires blood_spire in own front_line` | Blood Mist requires Blood Spire on your front line. | Blood Mist yêu cầu Blood Spire ở hàng trước phe bạn. | `lib_ability_aura.lua` |
| `ERR_ABILITY_BLOOD_MIST_ATK_REDUCTION_INVALID` | `blood_mist requires positive base_stats.atk_reduced` | Blood Mist requires a positive ATK reduction value. | Blood Mist yêu cầu chỉ số giảm ATK dương. | `lib_ability_aura.lua` |
...
| `ERR_ABILITY_BLOODMIGHT_NO_SYTHRA` | `bloodmight requires untriggered sythra in own front_line` | Bloodmight requires an untriggered Sythra on your front line. | Bloodmight yêu cầu Sythra chưa kích hoạt ở hàng trước phe bạn. | `lib_ability_aura.lua` |
...
| `ERR_ABILITY_LUX_MAXIMA_TARGET_NOT_EXPOSED` | `lux_maxima target must be an exposed configured Darkborn Aura` | Lux Maxima can target only an exposed configured Darkborn Aura. | Lux Maxima chỉ nhắm Aura Darkborn đã cấu hình và đang bị lộ. | `lib_ability_lightborn.lua` |
